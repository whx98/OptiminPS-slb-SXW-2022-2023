...
Ubase=12e3  # base value of voltage, unit:V, i.e. this is a 10 kV distribution network
Ibase=Sbase/Ubase/1.732  # unit: A
Zbase=Ubase/Ibase/1.732  # unit: Ω
path = r'C:\Users\29951\PycharmProjects\pythonProject4\SCE56.xlsx'
...
index=0
Obj=0
LineInf=netpara.iloc[:57,3:5]
NodeInf=netpara.iloc[:57,1:3]
...
'''
model = Model('mip')
u = model.addMVar((N,1),vtype=GRB.CONTINUOUS)
v = model.addMVar((N,1),vtype=GRB.CONTINUOUS)
P_ij= model.addMVar((L,1),vtype=GRB.CONTINUOUS)
Q_ij= model.addMVar((L,1),vtype=GRB.CONTINUOUS)
l_ij= model.addMVar((L,1),vtype=GRB.CONTINUOUS)
Pi= model.addMVar((N,1),vtype=GRB.CONTINUOUS)
Qi= model.addMVar((N,1),vtype=GRB.CONTINUOUS)
   ...
   #Path = nx.shortest_path(G, source=lt,target=1)
    ...
    #for j in range(len(Path)-1):
        #for k in range(L):
                ...
                #print( Path_L)
                #AE=A_l[i]
                #AE[:,:,j]=np.diag([1]*2)-2/v_min * np.transpose([r[k],x[k]]).dot([Pij_hat[k],Qij_hat[k]])
                #遍历2*2
