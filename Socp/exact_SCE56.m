...
WithSvolt=1; % 1 / 0 means with / without s\in S_volt
%% System Parameters
...
Cons_Load=[];
Eta=0.4;  % Scale up the PV penetration rate, 5 means 5 times than Table I in [1]
Pi_max=zeros(N,1);
Qi_max=zeros(N,1);
for i=1:N
...
if result.problem==0
    ...
    if WithSvolt==1
        title("Exact SOCP relaxation for DOPF: SCE 56 bus case with s \in S_{volt}")
    else
        title("SOCP relaxation for DOPF: SCE 56 bus case WITHOUT s \in S_{volt}")
    end
end