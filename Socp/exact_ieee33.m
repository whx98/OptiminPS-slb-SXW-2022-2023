...
netpara=xlsread('IEEE33bw','网络参数');
loadpoint=xlsread('IEEE33bw','节点负荷');
% L=size(netpara,1); % number of lines
L=32;
r=netpara(2:end,4)/Zbase;% resistance, unit: p.u.
x=netpara(2:end,5)/Zbase;% reactance, unit: p.u.
...
Pij_hat=abs(In(2:end,:)\Pi_max(2:end));
% Qij_hat=abs(inv(In(2:end,:))*Qi_max(2:end));
Qij_hat=abs(In(2:end,:)\Qi_max(2:end));
IJ=[I J];
...
if result.problem==0
    ...
    if WithSvolt==1
        title("Exact SOCP relaxation for DOPF: IEEE 33 bus case with s \in S_{volt}")
    else
        title("SOCP relaxation for DOPF: IEEE 33 bus case WITHOUT s \in S_{volt}")
    end
end