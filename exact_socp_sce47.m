...
h1=figure;
p=plot(GN);
highlight(p,find(Type==1),'Marker','s','NodeColor','c','Markersize',10);  % Capacitator
highlight(p,find(Type==0),'Marker','v','NodeColor','y','Markersize',10);  % PV panels
...
Cons=[Cons,Cons_V];
disp('Constraints on voltage calculation completed!')
% Cons_V
...
Cons=[Cons,Cons_SOC];
disp('Constraints on SOCP relaxation completed!')
% Cons_SOC
...
Pr_p=ones(1,5);
Obj=Pr_pv.*Pr_p*Pi([find(Type==0)])+Pr_sub*Pi(1);%目标函数
% C=sum(Pi); 
%% 模型求解
...
if result.problem==0
 %% 求解结果
    ...
    display(['分布式光伏电源',num2str(sum(s_Pi([find(Type==0)])),3),' MW']);
