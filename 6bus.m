...
% 取多场景下风力发电数据
Swinddata = xlsread("6bus.xlsx","多场景下的风电出力");
Swinddata = Swinddata(2:11,:);
Snum = size(Swinddata,1);
NT = size(Swinddata,2);

% 取线路电感 传输容量 起始节点 末端节点
busdata = xlsread("6bus.xlsx", "线路参数");
X = busdata(:,5);
Plimit = busdata(:,6);
NodeNum = 6;
I = busdata(:,2);
...
% 取线路有功和无功负荷
loaddata = xlsread("6bus.xlsx","预测负荷和风电出力");
loadP = loaddata(2,:);
loadQ = loaddata(3,:);
